...
string_split = input("Input string to split: ").lower().replace(" ", "")  # Takes given hex delimiter, converts to lowercase and strips all whitespace
...
for active_file in pathlib.Path(target_dir).glob('*' + file_type):  # Looks at every file in given folder ending with the given file type
	try:
		with open(active_file, "rb") as from_file, open(str(active_file).rsplit(".",1)[0] + "_audited." + str(active_file).rsplit(".",1)[1], "wb") as to_file:  # Opens up current file as a binary file, along with a new file that takes the original file and modifys it
			...
			bytes_to_audit = "".join(bytes_to_audit[1:])  # Readds everything after hex delimiter to the delimiter itself
			...
			print(str(active_file).rsplit(os.path.sep)[-1] + " was successfully pruned.")  # Prints out the name of the file pruned
